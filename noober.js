  ...
  for (let i = 0; i < json.length; i++) {
    if (json[i].length > 1) {
      ...
      for (let j = 0; j < json[i].length; j++) {
        passengerName =
          json[i][j].passengerDetails.first +
          [" "] +
          json[i][j].passengerDetails.last;
        ...
        passengerPickupAddressLine2 =
          json[i][j].pickupLocation.city +
          ", " +
          json[i][j].pickupLocation.state +
          ", " +
          json[i][j].pickupLocation.zip;
        passengerDropoffAddressLine1 = json[i][j].dropoffLocation.address;
        passengerDropoffAddressLine2 =
          json[i][j].dropoffLocation.city +
          ", " +
          json[i][j].dropoffLocation.state +
          ", " +
          json[i][j].dropoffLocation.zip;
      ...
      outputElement.insertAdjacentHTLM(
        "beforeend",
        `
        <h1 class="inline-block mt-8 px-4 py-2 rounded-xl text-2xl bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-500">
      <i class="fas fa-car-side"></i>
      <span>levelOfServiec</span>
    </h1>
        `
      );
      outputElement.insertAdjacentHTLM(
